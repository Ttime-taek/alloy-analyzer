    ...
    def _polish_rule_draft_with_ai(self, fb: dict, mode: str, comp_str: str, norm: dict) -> dict | None:
        ...
        if m == "lab":
            ...
            tone = (
                ...
                "IMC·금속간화합물은 '기판과 납땜 사이 단단한 막' 등으로 풀고, 화학식 약칭은 괄호에 한 번만."
            )
        prompt = f"""{tone}
...
"""
    ...
    def _rule_phase_consumer(self, norm):
        ...
        if cu > 0:
            if cu >= 0.7:
                ...
                lines.append("구리가 조금 있으면 구리와 주석이 만나 만든 단단한 막(약칭: Cu6Sn5)이 생겨 접합 강도에 도움이 됩니다.")
        if bi >= 40:
    ...
    def _build_local_fallback(self, norm, result, knn, literature_mode: str = "fast", mode: str = "eng"):
        ...
        if m == "eng":
            ...
            if not imc:
                imc = ["이 조성만으로는 어떤 단단한 막이 가장 두드러지는지 특정하기 어렵습니다."]
        ...
        if m == "eng":
            ...
            summary = (
                ...
                + "\n".join(eng_extra)
                + "\n- 아래 상세·막(IMC)·원소 설명은 규칙·DB로만 채웠습니다. Gemini 문단을 쓰려면 GEMINI_API_KEY·google-genai 설치·네트워크를 확인하세요. 키가 있는데도 이 표시면 쿼터 초과나 AI가 JSON이 아닌 답을 준 경우일 수 있습니다(화면의 AI 상태 문구 참고)."
                + lit_block_eng
            )
        else:
            summary = (
...
- 화학식(예: Cu6Sn5)은 문장마다 반복하지 말고, "구리와 주석이 만난 단단한 막(약칭: Cu6Sn5)" 처럼 한 줄에만.
...
반드시 한국어 JSON만 출력:
{{
 "phase": "5~10문장. 녹는 온도 구간, 기판과 납땜이 만나면 어떤 '단단한 막'이 생길 수 있는지 쉬운 말로. 전문 용어 최소화.",
