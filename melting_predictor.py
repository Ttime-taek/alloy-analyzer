...
# [2026-09-25 계산 로직 점검 P3] 아공정(Ag 1~3 %) 고상선 217 → 221 ℃.
#   Sn-Ag 이원계에서 고상선은 공정 온도(221 ℃)보다 낮을 수 없다. 217 ℃는 Cu가 있는 SAC 3원 공정값으로,
#   DB의 Cu 없는 Sn-Ag-P 행(Sn-0.3Ag-0.015P, Sn-3.0Ag-0.15P 고상 217)에 맞춰진 것으로 보인다.
#   해당 DB 행은 원출처 확인 대상(점검 보고서 참고). 이 표는 SAC 경로에선 액상선에만 쓰여 SAC 고상선은 무관.
SN_AG_PHASE = [
    (0.0,  231.9, 231.9),
    (1.0,  221.0, 227.0),
    (2.0,  221.0, 224.0),
...
ELEMENT_MP = {
    ...
    "Al": 660.3,  "Pd": 1554.9,
    # [2026-09-25 계산 로직 점검 P3] UI에서 입력 가능한데 빠져 있어 600 ℃로 대체되던 원소
    "Pt": 1768.3, "Fe": 1538.0, "Co": 1495.0, "Mn": 1246.0,
    "Mo": 2623.0, "Se": 221.0, "Te": 449.5, "P": 44.2,
}
